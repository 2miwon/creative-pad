        ...
        ZStack{
            List {
                ForEach(fileURLs, id: \.self) { fileURL in
                    ShareLink(item: MusicURL(text: fileURL.appendingPathComponent("Record").lastPathComponent).url) {
            ...
            .onAppear {
                DispatchQueue.global().async {
                    if let documentsUrl = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                        do {
                            let fileURLs = try FileManager.default.contentsOfDirectory(at: documentsUrl.appendingPathComponent("Record") , includingPropertiesForKeys: nil)
